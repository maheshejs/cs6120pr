...
type State = {
  ...
  // For tracing: trace file writer, tracing flag, and set of traced calls.
  traceWriter: Deno.FsFile | null;
  isTracing: boolean;
  tracedCalls: Set<string>;
  tracedFunctions: Map<string, {
    name: string;
    args: bril.Argument[];
    instrs: (bril.Instruction | bril.Label)[];
    type?: bril.Type;
  }>;
  currentTracedFunc: string | null;
  
  // Current function being executed (for getting return type).
  currentFunc: bril.Function | null;
};

/**
 * Rename a variable with cc-rename- prefix, unless it's a special cc-* variable.
 */
function renameVar(varName: string): string {
  // Don't rename special cc-* variables
  if (varName.startsWith("cc-")) {
    return varName;
  }
  return `cc-rename-${varName}`;
}

/**
 * Create a copy of an instruction for tracing (as JSON object) with renamed variables.
 */
function copyInstructionForTrace(instr: bril.Instruction): bril.Instruction {
  if (instr.op === "const") {
    return {
      op: "const",
      dest: renameVar(instr.dest),
      type: instr.type,
      value: instr.value,
    };
  } else if ("dest" in instr) {
    // Value operation
    const result: bril.ValueOperation = {
      op: instr.op,
      dest: renameVar(instr.dest),
      type: instr.type,
    };
    if (instr.args && instr.args.length > 0) {
      result.args = instr.args.map(renameVar);
    }
    if (instr.funcs && instr.funcs.length > 0) {
      result.funcs = [...instr.funcs];
    }
    if (instr.labels && instr.labels.length > 0) {
      result.labels = [...instr.labels];
    }
    return result;
  } else {
    // Effect operation
    const result: bril.EffectOperation = {
      op: instr.op,
    };
    if (instr.args && instr.args.length > 0) {
      result.args = instr.args.map(renameVar);
...
function evalCall(instr: bril.Operation, state: State): Action {
  ...
  // Check if we should start tracing this call
  const shouldTrace = state.traceWriter !== null && !state.tracedCalls.has(funcName);
  if (shouldTrace) {
    state.tracedCalls.add(funcName);
    state.currentTracedFunc = funcName;
    
    // Create traced function entry with renamed parameters
    const tracedFunc = {
      name: funcName,
      args: params.map(p => ({
        name: renameVar(p.name),
        type: p.type,
      })),
      instrs: [] as (bril.Instruction | bril.Label)[],
      type: func.type,
    };
    
    // Copy parameters from cc-param-0, cc-param-1, etc.
    for (let i = 0; i < params.length; i++) {
      const paramName = params[i].name;
      const paramType = params[i].type;
      tracedFunc.instrs.push({
        op: "id",
        dest: renameVar(paramName),
        type: paramType,
        args: [`cc-param-${i}`],
      });
    }
    
    state.tracedFunctions.set(funcName, tracedFunc);
  }
  ...
  const newState: State = {
    ...
    isTracing: shouldTrace,
    tracedCalls: state.tracedCalls,
    tracedFunctions: state.tracedFunctions,
    currentTracedFunc: shouldTrace ? funcName : state.currentTracedFunc,
    currentFunc: func,
  };
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  // Handle tracing: stop tracing on non-value operations, call, or ret
  if (state.isTracing && state.currentTracedFunc !== null) {
    const tracedFunc = state.tracedFunctions.get(state.currentTracedFunc);
    if (!tracedFunc) {
      state.isTracing = false;
      state.currentTracedFunc = null;
    } else if (instr.op === "call") {
      // Stop tracing on call
      state.isTracing = false;
      state.currentTracedFunc = null;
    } else if (instr.op === "ret") {
      // Handle ret: add cc-ret and cc-inl before stopping
      if (instr.args && instr.args.length > 0) {
        const retArg = instr.args[0];
        // Get the return type from the current function
        let retType: bril.Type = "int";
        if (state.currentFunc && state.currentFunc.type) {
          retType = state.currentFunc.type;
        } else {
          // Fallback: try to infer from the value
          const retVal = get(state.env, retArg);
          if (typeof retVal === "boolean") {
            ...
            retType = "char";
          } else if (typeof retVal === "object" && !(retVal instanceof BigInt)) {
            retType = { ptr: "int" }; // Default pointer type
          }
        }
        tracedFunc.instrs.push({
          op: "id",
      ...
      // Non-value operation - stop tracing
      state.isTracing = false;
      state.currentTracedFunc = null;
    } else {
      // Value operation (has dest) - trace it
      tracedFunc.instrs.push(copyInstructionForTrace(instr));
    }
  }
...
function evalFunc(func: bril.Function, state: State): Value | null {
  ...
  const prevTracedFunc = state.currentTracedFunc;
  const prevFunc = state.currentFunc;
  ...
  // Restore current function and tracing state
  state.currentFunc = prevFunc;
  if (!wasTracing) {
    state.currentTracedFunc = prevTracedFunc;
  }
...
function evalProg(prog: bril.Program) {
  ...
  if (tidx > -1) {
    if (tidx + 1 >= args.length) {
      throw error(`-t flag requires a filename`);
    }
    traceFile = args[tidx + 1];
    args.splice(tidx, 2);
  }
  ...
  let traceWriter: Deno.FsFile | null = null;
  if (traceFile !== null) {
    try {
      traceWriter = Deno.openSync(traceFile, { write: true, create: true, truncate: true });
    } catch (e) {
      if (e instanceof Error && e.name === "NotCapable") {
        throw error(
          `Cannot write to trace file "${traceFile}". ` +
          `Run with --allow-write flag: deno run --allow-write brili.ts -t ${traceFile} ...`
        );
      }
      throw e;
    }
  ...
  const state: State = {
    ...
    isTracing: false,
    tracedCalls: new Set(),
    tracedFunctions: new Map(),
    currentTracedFunc: null,
    currentFunc: null,
  };
  evalFunc(main, state);

  // Write trace file as JSON
  if (traceWriter !== null && state.tracedFunctions.size > 0) {
    const traceProgram: bril.Program = {
      functions: Array.from(state.tracedFunctions.values()),
    };
    const encoder = new TextEncoder();
    const jsonStr = JSON.stringify(traceProgram, null, 2);
    traceWriter.writeSync(encoder.encode(jsonStr));
    traceWriter.close();
